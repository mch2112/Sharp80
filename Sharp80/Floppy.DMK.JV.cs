                ...
                foreach (var sd in sectors)
                {
                    ...
                    if (sd.InUse)
                    {
                        if (DiskData.Length - diskCursor < sd.SectorSize) // not enough data for sector
                        {
                            ...
                            Log.LogDebug(string.Format("JV3 File has invalid length, can't fill track {0} sector {1}", sd.TrackNumber, sd.SectorNumber));
                        }
                        else
                        {
                        ...
                        if (j < sectors.Count)
                        {
                            ...
                            switch (ss.DAM)
                            {
                                ...
                                case DAM_NORMAL: // 0x00
                                    break;
                                default:
                                    Log.LogDebug(string.Format("Bad DAM ({0}) in {1}", ss.DAM, ss));
                                    break;
                            }
                            cursor++;
                        }
